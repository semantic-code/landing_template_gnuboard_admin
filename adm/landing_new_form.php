...
if($w == 'u'){
    ...
    if (isset($ld_page) && $ld_page) {
        $sql = "SELECT * FROM g5_landing WHERE ld_page = '{$ld_page}'";
...
?>
    ...
    <style>
        .cheditor-editarea-wrapper, .cheditor-editarea{height: 50rem!important;}
        ...
        .file-upload-box.add-box {
            cursor: pointer;
        }

        .file-upload-box.add-box label {
            position: relative;
        }

        .file-upload-box.add-box label span {
            position: absolute;
            top: 50%;
            left: 50%;
    ...
    <form name="frmLanding" id="frmLanding" action="./landing_form_update.php" method="post" onsubmit="return on_submit(this)">
        <input type="hidden" name="ld_page" value="<?php echo $landing['ld_page'] ?? ''; ?>">
        <input type="hidden" name="w" value="<?= $w ?>">
        <input type="hidden" name="chk_ld_page" value="0">
        ...
        <div class="tbl_frm01 tbl_wrap">
            <table>
                ...
                <tr style="">
                    ...
                    <td class="<?= $config['cf_editor'] ?>">
                        <div class="file-upload-wrapper">
                            <!-- 추가 버튼 -->
                            <div class="file-upload-box add-box">
                                <label for="file-input"><span>+</span></label>
                                <input type="file" name="bf_file[]" id="file-input" multiple accept="image/*">
                            </div>
                ...
                <tr>
                    <th scope="row"><label for="ld_fields">폼 필드 정의</label></th>
                    <td>
            ...
            $(document).on('change', '#file-input', function(){
                ...
                $.each(files, function(i, file){
                    ...
                    reader.onload = function(e){
                        const $box = $('<div>', {class : 'file-upload-box'});
                        const $img = $('<img>', {src : e.target.result});
                        const $remove_btn = $('<button>', {class : 'remove-btn', text : 'x'}).on('click', function(){
                            $box.remove();
                        });
